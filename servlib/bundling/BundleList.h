...
public:
     ...
     */
    typedef enum {
        SORT_PRIORITY = 0x01,	///< Sort by bundle priority
        SORT_FIFO = 0x02,       ///< Sort by FIFO policy
        SORT_LIFO = 0x03,       ///< Sort by LIFO policy
        SORT_METADATA_ASCENDING_ALPHABETICAL = 0x04,    ///< Sort by METADATA_ASC_ALPH policy
        SORT_METADATA_DESCENDING_ALPHABETICAL = 0x05,   ///< Sort by METADATA_DESC_ALPH policy
        SORT_ASCENDING_ALPHABETICAL = 0x06,     ///< Sort by ASC_ALPH policy
        SORT_DESCENDING_ALPHABETICAL = 0x07,    ///< Sort by DESC_ALPH policy
        SORT_PRIORITY_TABLE = 0x08,     ///< Sort by priority table
        SORT_FRAG_OFFSET	///< Sort by fragment offset
    } sort_order_t;
        
    /* 
     * sort the bundle of the deferred list with a specified policy
     */
    void sort_list(sort_order_t sort_order);
    
    /**
     * Insert the given reference bundle sorted by the given sort method.
     */
    void insert_sorted_bundle_reference(const BundleRef& bundle, 
        std::string& first_object, sort_order_t sort_order);
    
    /*
     *  Utility for insert the given reference bundle in the list.
     */
    void shift_and_insert(const BundleRef& bundle, 
        BundleList::iterator sorted_list_iter, BundleList *container);
    
    /*
     * Insert the given reference bundle sorted by the priority table
     */
    void insert_sorted_bundle_reference_priority_table(std::vector<BundleList*>lists,
