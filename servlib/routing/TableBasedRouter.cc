...
static const char* LOG = "/dtn/bundle/list";
...
TableBasedRouter::check_next_hop(const LinkRef& next_hop)
{
    ...
    /********************************** Tartarini *****************************/
    /* 
     * sort the bundle list according to the specified policy
     */
    if((config_.bundle_fwd_order_ == BundleList::SORT_PRIORITY_TABLE) &&
            (config_.prioritymap_.empty()))
    {
            log_err_p(LOG, "TableBasedRouter::check_next_hop: BundleRouter::config_.prioritymap_ is empty, "
                    "need to be initialized from file");
            return;
    }
    deferred->list()->sort_list(config_.bundle_fwd_order_);
    /**************************************************************************/
